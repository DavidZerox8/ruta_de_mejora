    ...
    success: function(res){
      ...
      if(res == 'error_1'){
        ...
        */
        ...
        swal('Error', 'El usuario y contraseña que ingresaste son incorrectos', 'error');
      }else{
...
$('#registro').click(function(){
  ...
  $.ajax({
    ...
    success: function(res){
      ...
      if(res == 'error_1'){
        swal('Error', 'Campos obligatorios, por favor llena el  nombre de usuario y las claves', 'warning');
      }else if(res == 'error_2'){
        swal('Error', 'Las claves deben ser iguales, por favor intentalo de nuevo', 'error');
      }else if(res == 'error_3'){
        swal('Error', 'El usuario o el grupo que ingresaste ya se encuentra registrado', 'error');
      }else if(res == 'error_4'){
        swal('Error', 'Por favor ingresa un grupo valido', 'warning');
      }else{
        window.location.href = res ;
      }
