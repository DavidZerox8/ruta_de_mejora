  ...
  class Usuario extends Conexion
  {
    ...
    public function login($user, $clave)
    {
      ...
      */
      ...
      $consulta = 'select id, nombre, grupo, cargo from usuarios where user="'.$user.'" and clave= MD5("'.$clave.'")';
      /*
      ...
      */
      ...
      if($verificar_usuario > 0){
        ...
        */
            ...
            $_SESSION['nombrec'] = "";
            $_SESSION['asistenciaj'] = "";
            $_SESSION['asistencianj'] = "";
    ...
    public function registroUsuario($name, $user, $grupo, $clave)
    {
      parent::conectar();

      $name  = parent::filtrar($name);
      $user = parent::filtrar($user);
      $grupo = parent::filtrar($grupo);
      $clave = parent::filtrar($clave);


      // validar que el correo no exito
      $verificarUsuario = parent::verificarRegistros('select id from usuarios where user="'.$user.'" ');
      $verificarUsuario2 = parent::verificarRegistros('select id from usuarios where grupo="'.$grupo.'" ');

      if($verificarUsuario > 0 ){
        echo 'error_3';
      } 
        elseif($verificarUsuario2 > 0)
        {
           echo 'error_3';   
        }
        else{

            parent::query('insert into usuarios(nombre, user, grupo, clave, cargo) values("'.$name.'", "'.$user.'", "'.$grupo.'", MD5("'.$clave.'"), 2)');

            header("Location: view/admin/index.php"); /* Redirección del navegador */
